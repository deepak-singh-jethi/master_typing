...
import { Award, Flame, Moon, Sun } from "lucide-react";
...
function CourseArc({ value }) {
  ...
  return (
    <svg viewBox="0 0 120 66" className="mt-1 h-16 w-full" aria-hidden="true">
      <path d="M14 56 A48 48 0 0 1 106 56" pathLength="100" fill="none" stroke="currentColor" strokeWidth="9" strokeLinecap="round" className="text-slate-200 dark:text-slate-800" />
      <path d="M14 56 A48 48 0 0 1 106 56" pathLength="100" fill="none" stroke="currentColor" strokeWidth="9" strokeLinecap="round" strokeDasharray={`${progress} 100`} className="text-violet-600" />
    </svg>
  );
}
...
export function Sidebar() {
  ...
  return (
    <aside className={cn(
      "fixed inset-y-0 left-0 z-30 hidden w-56 border-r border-slate-200/80 bg-white px-4 py-5 lg:flex lg:flex-col dark:border-slate-800 dark:bg-slate-950",
      focusedLesson && "dark:bg-[#07101d]",
    )}>
      ...
      {focusedLesson ? (
        <div className="mt-auto space-y-3">
          <Link to="/learn" aria-label={`Open course, ${Math.round(courseProgress)} percent complete`} className="group block overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 transition hover:border-violet-300 dark:border-slate-800 dark:bg-[#0a1422] dark:hover:border-violet-500/40">
            ...
            <div className="border-t border-slate-200 px-4 py-3 text-[11px] text-slate-500 dark:border-slate-800 dark:text-slate-400">
              {data.progress.completedLessons.length} / {course.totalLessons || 27} lessons mastered
            </div>
          ...
          <div className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-[#0a1422]">
            <div className="px-4 py-3.5">
              <div className="flex items-center gap-2 text-xs font-semibold text-slate-700 dark:text-slate-200"><Flame className="size-4 text-orange-500" />Current streak</div>
              <p className="mt-2 text-lg font-semibold text-slate-950 dark:text-white">{streak} {streak === 1 ? "day" : "days"}</p>
              <div className="mt-3 grid grid-cols-7 gap-1 text-center text-[9px] font-medium text-slate-400">
                {["S","M","T","W","T","F","S"].map((day, index) => (
                  <div key={`${day}-${index}`}>
                    <span>{day}</span>
                    <span className={cn("mx-auto mt-2 block size-2 rounded-full", index === new Date().getDay() ? "bg-violet-500 ring-4 ring-violet-500/10" : "bg-slate-200 dark:bg-slate-700")} />
                  </div>
                ))}
              </div>
            </div>
            <div className="border-t border-slate-200 px-4 py-3.5 dark:border-slate-800">
              ...
              <p className={cn("mt-2 text-xs font-semibold", goalComplete ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400")}>{goalComplete ? "✓ Completed" : `${Math.max(0, goalMinutes - todayMinutes)} min remaining`}</p>
            </div>
          </div>
          ...
          >
            <span className="inline-flex items-center gap-2">{resolvedTheme === "dark" ? <Moon className="size-4" /> : <Sun className="size-4" />}Dark mode</span>
            <span className={cn("relative h-5 w-9 rounded-full transition", resolvedTheme === "dark" ? "bg-violet-600" : "bg-slate-300")}>
        ...
        <Link to="/learn" aria-label={`Open course, ${Math.round(courseProgress)} percent complete`} className="group mt-auto rounded-2xl border border-slate-200 bg-slate-50 p-3.5 transition hover:border-indigo-300 hover:bg-indigo-50/50 dark:border-slate-800 dark:bg-slate-900/70 dark:hover:border-indigo-500/40 dark:hover:bg-indigo-500/5">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2 text-xs font-semibold text-slate-600 dark:text-slate-300"><Award className="size-4 text-indigo-500" aria-hidden="true" />Course</div>
