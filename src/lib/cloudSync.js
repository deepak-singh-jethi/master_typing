...
export function toSessionPayload(item, userId = undefined) {
  const row = {
    ...
    metadata: {
      ...
      reviewAttempt: item.reviewAttempt,
      sessionPassed: item.sessionPassed,
      reviewPolicyVersion: item.reviewPolicyVersion,
      reviewSessionVersion: item.reviewSessionVersion,
      reviewCycleId: item.reviewCycleId,
      reviewOutcome: item.reviewOutcome,
      reviewAccuracyTarget: item.reviewAccuracyTarget,
      reviewFocusErrorLimit: item.reviewFocusErrorLimit,
      reviewFocusErrorRate: item.reviewFocusErrorRate,
      reviewNextIntervalDays: item.reviewNextIntervalDays,
...
function mergeMastery(first = {}, second = {}) {
  ...
  return {
    ...
    reviewAttempts: Math.max(number(first.reviewAttempts), number(second.reviewAttempts)),
    exerciseResults,
